  ...
  try {
    ...
    try {
      ...
      if (isNaN(dateObj.getTime())) {
        // 如果日期无效，使用当前日期
        formattedDate = new Date().toLocaleDateString(resolvedParams.locale, {
          year: 'numeric',
          month: 'long',
          day: 'numeric',
        })
      } else {
        formattedDate = dateObj.toLocaleDateString(resolvedParams.locale, {
          year: 'numeric',
          month: 'long',
          day: 'numeric',
        })
    ...
    try {
      ...
      datePublished = isNaN(pubDate.getTime()) ? new Date().toISOString() : pubDate.toISOString()
      
      const modDate = new Date(post.updatedAt || post.date)
      dateModified = isNaN(modDate.getTime()) ? datePublished : modDate.toISOString()
    } catch (error) {
